# Google Antigravity Usage & Quota Monitor (Menu Bar & Dock App)
<!-- v3 – Added native macOS menu bar status item, live percentage display, and context menu -->

A lightweight, zero-configuration utility, native macOS desktop/menu bar application, and cross-device network daemon to view real-time Google Antigravity (`agy`) model quotas, consumption percentages, and reset timers across all your devices.

---

## 🌟 macOS Menu Bar & Dock Integration ("Click and Tada")

The application lives simultaneously in your **macOS Menu Bar** and your **Dock**:

- **Menu Bar Status Item**: Displays a live indicator (e.g. `✦ 95%`) at the top-right of your screen.
  - **Left-Click**: Toggles the sleek dark-mode quota dashboard right below the menu bar item.
  - **Right-Click**: Opens a quick context menu showing per-model quota numbers, manual refresh, copy remote `curl` command, and quit.
  - **Live Auto-Update**: Background timer refreshes the percentage and status tooltip every 30 seconds.
- **Dock Icon**: Sits directly next to `Antigravity` in your Dock. Clicking it unminimizes or brings the dashboard window to front.
- **Background Daemon**: Starts automatically and connects to the local LaunchAgent on port 3007.
...
#### Query the Background Server Daemon
