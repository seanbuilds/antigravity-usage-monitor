#!/usr/bin/env python3
# v1 – Initial cross-device Antigravity usage and quota monitor CLI & server
"""
antigravity_usage_v1.py
...
def fetch_usage_data(access_token: str = None, refresh_token: str = None) -> dict:
    ...
    Handles token refreshing and host fallback automatically.
    """
    if not access_token:
    ...
    for attempt in range(2):  # Try initial, then refresh if 401
        for host in HOSTS:
            try:
                ...
                tier_info = load_resp.get("currentTier", {})
                tier_id = tier_info.get("id", "standard")
                upgrade_uri = tier_info.get("upgradeSubscriptionUri", "")
...
def render_dashboard(data: dict, color: bool = True) -> str:
    ...
    lines.append(f"  {bold}Account:{reset}  {account}   {dim}(Tier: {tier}){reset}")
...
class QuotaServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    cached_data = None
    last_fetch_time = 0
    cache_ttl = 60  # Cache for 60 seconds to avoid hitting API rate limits
    secret_token = None

    def log_message(self, format, *args):
        # Suppress routine access logs unless verbose
    ...
    def do_GET(self):
        ...
        # Check User-Agent or Accept header: if curl/terminal, output ANSI colors
        ua = self.headers.get("User-Agent", "").lower()
        use_color = ("curl" in ua or "wget" in ua or "httpie" in ua or "term" in parsed.query)
        # Even if not curl, plain text dashboard is readable in any browser/client
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Quick terminal view on current device:
  python3 antigravity_usage_v1.py

  # Live watch mode (refreshing every 30 seconds):
  python3 antigravity_usage_v1.py --watch 30

  # Output machine-readable JSON:
  python3 antigravity_usage_v1.py --json

  # Host cross-device server so any terminal on your network can check usage:
  python3 antigravity_usage_v1.py --serve --port 3007

  # Query a remote Antigravity server running on another machine:
  python3 antigravity_usage_v1.py --remote http://192.168.1.50:3007
  # Or simply from any terminal on any device:
  curl -s http://192.168.1.50:3007
"""
    )
