#!/usr/bin/env bash
# v3 – Build and install Antigravity Usage.app with Menu Bar Snap-Out Popover architecture
set -e
...
echo "==> Compiling native Swift executable (v3 Menu Bar Snap-Out Popover)..."
swiftc -O \
  -framework Cocoa \
  -framework WebKit \
  "${SCRIPT_DIR}/app_main_v3.swift" \
...
# Copy HTML UI (v3 compact popover layout)
cp "${SCRIPT_DIR}/index_v3.html" "${APP_DIR}/Contents/Resources/index.html"
...
echo "==> Launching updated Antigravity Usage (v3 Snap-Out Popover)..."
open "${APP_DIR}"

echo "==> Done! Antigravity Usage now snaps directly out from the Menu Bar."
